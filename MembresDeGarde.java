...
import com.sifast.stage.controller.Csv;
...
public class MembresDeGarde extends JFrame {
	...
	public String[] bloc = { "Pneumo", "Cardio", "Neuro" };
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public JComboBox Combo = new JComboBox(bloc);
	...
	public MembresDeGarde() {
		...
		/*
		 * // Set up tool tips for the sport cells. DefaultTableCellRenderer
		...
		ActionListener actionListener = new ActionListener() {
			public void actionPerformed(ActionEvent actionEvent) {
				...
				resultat = (String) Combo.getSelectedItem();

				System.out.println(", Position: " + Combo.getSelectedIndex());
			}
		};
		Combo.addActionListener(actionListener);
		resultat = bloc[Combo.getSelectedIndex()];
		...
		btnPlanningPDF.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (table.getValueAt(0, 0) == null)
					...
					// String bloc = (String) combo.getSelectedItem();

					// if(bloc.equals("Pneumologie"))
					...
					/*
					 * for(int i=0;i<4;i++){ if(tab[i].equals (bloc)){ try {
					 * 
					 * PdfGenerator.generatePdfFileNeurologie();
					 * 
					 * } catch (Exception ex) {
					 * Logger.getLogger(MembresDeGarde.class.getName()).log(
					 * Level.SEVERE, null, ex); } } else { i++; if
					 * (bloc.equals("Pneumologie")) { try {
					 * 
