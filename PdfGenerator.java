...
public class PdfGenerator {
	...
	public static void generatePdfFile() throws Exception {
		...
		document.add(new Paragraph("du " + String.format("%1$td/%1$tm/%1$tY", Service.plan.getDateDebut().getDate())
		...
		for (Object elem1 : MembresDeGarde.dates) { // na3mlou test
			...
			while (test2) {
				...
				if (!(docteur.getPreference().containsKey(elem1))) // champ
																	...
																	// dispo
				{
					document.add(new Paragraph("Le " + elem1 + ", le docteur:  "
							+ Service.docteurs.get(indice % Service.docteurs.size()).getNom() + " en garde "
							+ M.resultat));
					System.out.println(Service.docteurs.get(indice % Service.docteurs.size()).getNom() + M.resultat);
					indice++;
					test2 = false;
				} else {
					if (docteur.getPreference().get(elem1).equals(PrefEnum.not_dispo)) {
						...
						for (int i = 0; i < Service.docteurs.size(); i++) {
							if (!(Service.docteurs.get(i).getPreference().containsKey(elem1))) {
								document.add(new Paragraph("Le " + elem1 + ", le docteur:  "
										+ Service.docteurs.get(i).getNom() + " en garde " + M.resultat));
								System.out.println(
										Service.docteurs.get(indice % Service.docteurs.size()).getNom() + M.resultat);
								test = true;
								// indice++;
								test2 = false;
								break;
		...
		try {
			...
			if ((new File("F:\\work\\projects\\PlanningDesGardesMedicales\\planning.pdf")).exists()) {

				Process p = Runtime.getRuntime().exec(
						"rundll32 url.dll,FileProtocolHandler F:\\work\\projects\\PlanningDesGardesMedicales\\planning.pdf");
				p.waitFor();
